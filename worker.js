...
async function generateChatterboxAudio(text, language, audioPath) {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (!audio) {
        throw new Error("لم يتم استلام صوت من الخدمة.");
      }
      ...
      lastError = err;
      console.warn(
        `محاولة ${attempt + 1} فشلت: ${err.message}`
      );
  ...
  throw lastError || new Error("فشل توليد الصوت بعد كل المحاولات.");
}
...
function mergeWavFiles(wavBuffers) {
  ...
  let totalDataSize = totalSilence;
...
function parseWav(buffer) {
  ...
  if (!fmtChunk || !dataChunk) {
    throw new Error("تعذر قراءة بيانات WAV.");
  }
